...
class Stage8Signoff(BaseStage):
    ...
    def _evaluate_18_scorecard_checks(self, target: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        checks.append({
            "name": "Fuzzing",
            "score": 5.0,
            "reason": "Dynamic DAST and API fuzzing integrated via OmniSec Stage 4 & 6.",
        ...
        checks.append({
            "name": "SAST",
            "score": 10.0,
            "reason": "Static application security testing automated via OmniSec Stage 3.",
            "remediation": "Embed `omnisec scan --stages 3` into CI pull request check gates."
        })
